...
fn main() -> ! {
    ...
    // ブザー
    let speaker_pin = io.pins.gpio25.into_push_pull_output();
    ...
    // PWMを使用したいピンをアクティベートする
    let mut pwm_pin = mcpwm
        .operator0
        .with_pin_a(speaker_pin, PwmPinConfig::UP_ACTIVE_HIGH);

    // 内蔵スピーカの音が出る周波数の最低値
    // start timer with timestamp values in the range of 0..=99 and a frequency of 20 kHz
    let timer_clock_cfg = clock_cfg
        .timer_clock_with_frequency(98, PwmWorkingMode::UpDown, 800_u32.Hz())
        .unwrap();
    mcpwm.timer0.start(timer_clock_cfg);
    // pin will be high 50% of the time
    pwm_pin.set_timestamp(1);
