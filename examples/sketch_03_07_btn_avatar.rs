...
// 描画
use eg::{draw_target::DrawTarget, pixelcolor::Rgb565, prelude::*, primitives::Rectangle};
use embedded_graphics as eg;

mod avatar;
use avatar::{Avatar, Emotion};
...
fn main() -> ! {
    ...
    // アバター
    let mut avatar: Avatar = Avatar::new(&mut display);
    delay.delay_ms(1_000_u32);

    // ボタン
    let pin_btn_a = io.pins.gpio39.into_pull_up_input();
    let pin_btn_b = io.pins.gpio38.into_pull_up_input();
    let pin_btn_c = io.pins.gpio37.into_pull_up_input();
    ...
    loop {
        // ボタンを押すと表情を切り替える
        if pin_btn_a.is_low().unwrap() {
            avatar.change_emotion(&mut Emotion::Happy, &mut display);
        } else if pin_btn_b.is_low().unwrap() {
            avatar.change_emotion(&mut Emotion::Neutral, &mut display);
        } else if pin_btn_c.is_low().unwrap() {
            avatar.change_emotion(&mut Emotion::Sad, &mut display);
        }
    }
    // ここまでloop処理
}
